...
def replace_between(content, start_str, end_str, new_str, include_start=True, include_end=True):
    start_idx = content.find(start_str)
    if start_idx == -1: return content
    end_idx = content.find(end_str, start_idx + len(start_str))
    if end_idx == -1: return content
    
    pre = content[:start_idx] if not include_start else content[:start_idx + len(start_str)]
    post = content[end_idx + len(end_str):] if not include_end else content[end_idx:]
    
    return pre + new_str + post


# 1. We need to update renderProductTabsUI. Let's extract the whole function using standard brace matching
...
s, e = extract_function(html, "renderProductTabsUI")
if s != -1:
    old_func = html[s:e]
    # We will completely replace renderProductTabsUI to handle the Generated Quotes in Contacted stage
    new_func = """function renderProductTabsUI(isContacted, isReadOnly = false) {
      ...
      if (!isContacted && !isReadOnly) {
          // New Stage - Show Generate Button
          tabBox += `
            ...
            <button style="width: 100%; height: 44px; background: #473391; color: white; border: none; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer;" onclick="handleGenerateQuoteFromNew('${activeTab}')">
              Generate Quote →
            </button>
          `;
      } else if (isContacted) {
          // Contacted Stage - Show Generated Quotes List
              ...
              return `
                  <div style="border: 1px solid #E5E7EB; border-radius: 8px; padding: 12px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; background: #FAFAFA;">
                      ...
                      <div style="display: flex; align-items: center; gap: 16px;">
                          ...
                          ${isReadOnly ? '' : `<button style="background: white; border: 1.5px solid #473391; color: #473391; border-radius: 6px; padding: 6px 12px; font-size: 11px; font-weight: 700; cursor: pointer;" onclick="shareSingleQuote('${q.insurer}', '${q.plan}', '${q.premium}')">Share</button>`}
                      </div>
                  </div>
              `;
          }).join('');
          ...
          tabBox += `
            <div style="margin-top: 32px; border-top: 1px dashed #E5E7EB; padding-top: 24px;">
                ...
                ${quotesHtml}
                ${isReadOnly ? '' : `
                <button style="width: 100%; height: 44px; background: white; color: #473391; border: 1.5px solid #473391; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer; margin-top: 8px;" onclick="shareAllQuotes('${activeTab}')">
                  Share All Quotes →
                </button>
                `}
            </div>
...
# 2. Let's fix handleGenerateQuoteFromNew
s, e = extract_function(html, "handleGenerateQuoteFromNew")
if s != -1:
    old_func = html[s:e]
    new_func = """function handleGenerateQuoteFromNew(tab) {
        if (tab === 'lp') {
            showToast('Generating quotes via internal calculator...');
        } else {
            showToast('Simulating PolicyBazaar iframe scraping...');
            // In a real app we'd open the iframe, scrape, then proceed
        }
...
# 3. Add the Share methods (shareSingleQuote, shareAllQuotes)
share_funcs = """
    window.shareSingleQuote = function(insurer, plan, premium) {
        currentLead.sharedQuotes = [{ insurer, plan, premium }];
        proceedToQuoteBucket('Single quote shared');
    };
    
    window.shareAllQuotes = function(tab) {
        if (tab === 'lp') {
            currentLead.sharedQuotes = [
                { insurer: 'CARE Health', plan: 'Group Credit Protect Plus', premium: '12,500' },
                { insurer: 'TATA AIG', plan: 'Loan Secure', premium: '14,200' }
            ];
        } else {
        ...
        proceedToQuoteBucket('All generated quotes shared');
    };
    
    window.proceedToQuoteBucket = function(msg) {
        showSuccessToast('Quotes shared with customer via WhatsApp/Email');
        currentLead.bucket = 'Quote';
        currentLead.status = 'Quote Shared';
        currentLead.subStatus = 'Consent Pending';
        currentLead.quotePaymentShared = false; // Reset for new quote stage
        currentLead.log.unshift({ dot: '#10B981', text: `Agent Update · ${msg} · Just now`, time: 'Just now' });
        renderLeadDetail();
        if(typeof renderRightPanel === 'function') renderRightPanel();
    };
"""
if "window.shareSingleQuote =" not in html:
    html = html.replace('function renderQuoteStage(isReadOnly = false) {', share_funcs + '\n    function renderQuoteStage(isReadOnly = false) {')
