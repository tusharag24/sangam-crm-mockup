...
# 1. Inject MILESTONE_CONFIG and FREELOOK_DAYS globally
config_str = """
    const FREELOOK_DAYS = {
...
# Insert right after `// Init` or `<script>` tag
if '// === APP STATE ===' in html:
    html = html.replace('// === APP STATE ===', config_str + '\n    // === APP STATE ===')
else:
    print("Cannot find APP STATE marker")

# 2. Re-write renderPaymentStage and related functions to match Section 4
old_payment_stage_regex = r'    function renderPaymentStage\(\) \{.*?(?=    function render[A-Za-z]+\(\) \{)'
# We will just replace it completely if it exists. Wait, it's safer to just replace `function renderPaymentStage` block.
# Let's extract the block with a custom parser because regex for nested braces is brittle.
...
# Also ensure getMilestoneKey exists
get_milestone_key_str = """
...
"""
if "window.getMilestoneKey" not in html:
    html = html.replace('// === APP STATE ===', get_milestone_key_str + '\n    // === APP STATE ===')
