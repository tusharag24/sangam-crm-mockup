...
for tc in tool_calls:
    ...
    if name in ['replace_file_content', 'default_api:replace_file_content']:
        if 'TargetContent' in args:
            lines = apply_replace(lines, args['StartLine'], args['EndLine'], args['TargetContent'], args.get('ReplacementContent', ''))
            applied_count += 1
        ...
        for chunk in chunks:
            if 'TargetContent' in chunk:
                lines = apply_replace(lines, chunk['StartLine'], chunk['EndLine'], chunk['TargetContent'], chunk.get('ReplacementContent', ''))
        applied_count += 1
